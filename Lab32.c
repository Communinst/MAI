...
EXIT_CODE vector_init(int n, vector* vec, ...) 
{
    if (n <= 0) 
    {
        return INVALID_ARG;
...
EXIT_CODE print_vector(vector* vec) 
{
...
EXIT_CODE vector_dest(vector* vec) 
{
    if (!vec) 
    {
        return INVALID_ARG;
...
EXIT_CODE norm(vector* vec, double* nrm) 
{
    double mx = 0.0;
    
    for (UI i = 0; i < vec->n; i++) 
    {
...
EXIT_CODE norm_p(vector* vec, UI p, double* nrm) 
{
    if (p < 1) 
    {
        return INVALID_ARG;
...
EXIT_CODE matrix_multipl(double** A, double* x, double** res, int n) 
{
...
EXIT_CODE norm_a(vector* vec, double* nrm) 
{
...
EXIT_CODE longest_norm(EXIT_CODE (* norm_1)(vector*, double*), UI p,
                     EXIT_CODE (* norm_2)(vector*, UI, double*), EXIT_CODE (* norm_3)(vector*, double*),
...
int main() 
{
    ...
    if (vector_init(n, &vec_2, 1.0, 1.0, 0.5) == BAD_ALLOC) 
    {
        vector_dest(&vec_1);
        printf("Memory allocation went wrong\n");
        return BAD_ALLOC;
    ...
    if (vector_init(n, &vec_3, 1.0, 1.0, -1.51) == BAD_ALLOC) 
    {
        vector_dest(&vec_1);
        vector_dest(&vec_2);
    ...
    EXIT_CODE code;
    code = longest_norm(norm, 3, norm_p, norm_a, &size_1, &size_2, &size_3, &res_1, &res_2, &res_3, 3, vec_1,
                        vec_2, vec_3);
    if (code != OK) 
    {
        vector_dest(&vec_1);
