...
EXIT_CODE vectorPrint(vectorInt *v) {
    ...
    while (iterator != v->num_of_el) {
        if (v->el[iterator].exist == DONT_EXIST) {
            ...
            printf("\t%llfx^2 + (%llf)x + (%llf)\n", v->el[iterator].coefs[0], v->el[iterator].coefs[1], v->el[iterator].coefs[2]);
            printf("x1 = %llf\tx2 = %llf\n", v->el[iterator].solution1, v->el[iterator].solution1);
        }
        iterator++;
    }
}
...
EXIT_CODE stringValidInt (int argc, char* argv[]) {
    ...
    while (i < argc) {
        for (int step = 0; step < strlen(argv[i]); step++) {
            if ((argv[i][step] < '0' || argv[i][step] > '9')) {
                return INVALID;
            }
        }
        i++;
...
EXIT_CODE qtArgcCheck(int argc, char *argv[]) {
    ...
    if (stringValidDouble(argc, argv) != OK){
        return INVALID;
    }
    return OK;
}
...
EXIT_CODE qFunc(int argc, char* argv[], vectorInt *solutions) 
{
    ...
    if (qtArgcCheck(argc, argv) != OK) 
    {
        return (qtArgcCheck(argc, argv));
    }
...
EXIT_CODE mArgcCheck(int argc, char *argv[]) {
    ...
    if (stringValidInt(argc, argv) != OK){
        return INVALID;
    }
    return OK;
}
